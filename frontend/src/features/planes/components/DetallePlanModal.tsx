      ...
      <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] flex flex-col shadow-xl border border-slate-200/80 overflow-hidden min-h-0 animate-in zoom-in-95 duration-150">
        ...
        <div ref={modalDetalleScrollRef} className="p-6 space-y-6 flex-1 overflow-y-auto min-h-0 modal-scroll-body">
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-3">
            <div>
              <span className="font-bold uppercase text-slate-500 tracking-wider text-xs block mb-0.5">Diagnóstico Situacional:</span>
              <p className="text-slate-800 text-sm font-medium">{planActual.diagnosticoSituacional}</p>
            </div>
            <div>
              <span className="font-bold uppercase text-slate-500 tracking-wider text-xs block mb-0.5">Acciones Acordadas:</span>
              <p className="text-slate-800 text-sm font-medium">{planActual.accionesAcordadas}</p>
            </div>
            {planActual.compromisoPadres && (
              <div>
                <span className="font-bold uppercase text-slate-500 tracking-wider text-xs block mb-0.5">Compromiso Padres / Familia:</span>
                <p className="text-slate-800 text-sm font-medium">{planActual.compromisoPadres}</p>
              </div>
            )}
            {planActual.recomendacionesIa && (
              <div className="p-3 bg-amber-50 rounded-lg border border-amber-200 text-amber-900">
                ...
                <p className="text-xs leading-relaxed">{planActual.recomendacionesIa}</p>
              </div>
            )}
          </div>
            ...
            {(!planActual.seguimientos || planActual.seguimientos.length === 0) ? (
              ...
              <div className="space-y-2.5">
                {planActual.seguimientos.map((seg) => (
                  <div key={seg.id} className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                    <div className="flex items-center justify-between text-xs text-slate-500 mb-1">
                      <span className="font-bold text-slate-800 text-sm">{seg.usuarioNombre || 'Orientador'}</span>
                      <span>{new Date(seg.fechaRegistro).toLocaleString()}</span>
                    </div>
                    <p className="text-slate-700 text-sm">{seg.observacion}</p>
                  </div>
                ))}
              </div>
            )}
          ...
          <form onSubmit={handleRegistrarSeguimiento} className="p-4 bg-trujillo-ice/50 rounded-xl border border-trujillo-sky/30 space-y-4">
            <div className="flex items-center gap-2 text-sm font-bold text-trujillo-navy">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full p-2.5 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-trujillo-sky/30 focus:border-trujillo-sky bg-white"
                required
              />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  className="w-full h-10 px-3 rounded-xl border border-slate-300 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-trujillo-sky/30 text-slate-800 font-medium"
                >
            ...
            <div className="flex justify-end pt-2">
              <button
                ...
                className="h-10 px-4 rounded-xl bg-trujillo-navy hover:bg-trujillo-dark text-white text-sm font-semibold flex items-center gap-2 disabled:opacity-50 transition-all active:scale-95 shadow-sm cursor-pointer"
              >
        ...
        <div className="px-6 py-3.5 bg-slate-50 border-t border-slate-200 flex items-center justify-between shrink-0">
          ...
          <button
            type="button"
            onClick={onClose}
            className="h-9 px-4 text-sm font-semibold text-slate-700 hover:bg-slate-200/70 bg-white border border-slate-300 rounded-lg transition shadow-2xs cursor-pointer"
          >
            Cerrar Detalle
          </button>
        </div>
